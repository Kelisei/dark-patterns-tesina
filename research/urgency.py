...
urgency_matcher.add(
    ...
    [
        ...
        [
            ...
            {"LEMMA": "válido"},
            {"LOWER": "hasta"},
            {"LOWER": {"IN": ["medianoche", "mediodía", "hoy", "mañana", "noche"]}},
        ],
    ],
)
...
class UrgencyPredictorNLP(DarkStrategy):
    def predict(self,text: str):
        return check_text_urgency(text)

    def predict_multiple(self,texts: pd.Series):
        predictions = []
        for text in texts:
            predictions.append(check_text_urgency(text))
        return predictions